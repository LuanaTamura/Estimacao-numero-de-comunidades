...
#####
#Autovalores convergindo para Tracy Widom quando matriz de adjacencia segue Poisson
#####
...
sample.g <- function(P,n){
  ...
  for(i in 1:n){
    for(j in i:n){
      mat.adj[i,j] <- mat.adj[j,i] <- rpois(1,P)
    }
  }
...
L_min1 <- read.table("0. L_min n=100.txt")
L_max1 <- read.table("0. L_max n=100.txt")
L_min2 <- read.table("0. L_min n=300.txt")
L_max2 <- read.table("0. L_max n=300.txt")
L_min3 <- read.table("0. L_min n=1000.txt")
L_max3 <- read.table("0. L_max n=1000.txt")
