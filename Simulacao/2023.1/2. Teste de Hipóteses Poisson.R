...
source("D:/UFSCar - USP/Dissertação - Andressa Cerqueira/Códigos do R/2023.1/1. codigo_sbm poisson.R", local=TRUE)

#####
#Teste de hipóteses estimando K comunidades com spectral cluster
#####
...
#test.hip <- function(n){
  ...
  #vetor z que dara as comunidades de cada vertice
  ...
  GoFStat <- function(A, K0) {
    ...
    Theta.hat <- Generate.theta(n, K0, table(clusters)) #matriz para aux?lio nos calculos
    P.hat <- Theta.hat %*% B.hat %*% t(Theta.hat)
    tilde.A <- (A - P.hat + diag(diag(P.hat))) / sqrt((n-1)*P.hat)
    lambda <- slanczos(tilde.A, k = 1, kl = 1)$values
    lambda.1 <- lambda[1]
    lambda.n <- -lambda[2]
    sig.1 <- max(abs(lambda))
    return(list(n^(2/3) * (c(lambda.1, lambda.n, sig.1) - 2)))
  }
  ...
  SeqTestSBM <- function(A, alpha = 0.0001, K.min = 1, K.max = 100, n.dim = NULL, n.b = 50, use.boot = T) {
    n <- nrow(A)
  ...
  Generate.B <- function(K, low.limit = 0.1, upp.limit = 0.9) {
    B <- matrix(low.limit+ (upp.limit-low.limit)*rpois(K^2), ncol = K)
    tB <- t(B)
  ...
  Generate.psi <- function(n, K, clust.size, DCBM, low.limit = 0.2, upp.limit = 1){
    if (DCBM) {
      psi <- low.limit + (upp.limit-low.limit)* rpois(n)
      for (k in 1:K){
        if (k==1){id1 <-1} else {id1 <- sum(clust.size[1:(k-1)])+1}
        id2 <- sum(clust.size[1:k])
